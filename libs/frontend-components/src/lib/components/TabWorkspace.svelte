<script lang="ts">
    ...
    import { createWorkspace, type WorkspaceInitialSplit, activateTab, deletePane, moveTab, resizeSplit, splitPane, workspaceLayout, type DividerLayout, type WorkspaceNode } from './tabWorkspace.js'
    let { tabs, children, selected = $bindable<string>(), label = 'Workspace', initialSplit }: {
    ...
    let root: WorkspaceNode = $state(untrack(() => createWorkspace(tabs.map(tab => tab.id), selected, initialSplit)))
    let layout = $derived(workspaceLayout(root))
    ...
    let resize: { id: string; start: number; extent: number; ratio: number; axis: 'horizontal' | 'vertical' } | undefined
    $effect(() => {
        const tab = selected
        if (tab) untrack(() => { root = activateTab(root, tab) })
    })
    function select(tab: string) { root = activateTab(root, tab); selected = tab }
    function transfer(tab: string, pane: string, beforeTab?: string) {
        root = moveTab(root, tab, pane, beforeTab)
        selected = tab
...
<div class="workspace" aria-label={label} bind:this={element}>
    {#each layout.panes as item (item.pane.id)}
        <section class="pane" class:drop-target={dropTarget === item.pane.id}
            aria-label={`${label} pane ${layout.panes.indexOf(item) + 1}`}
            ...
            <header>
                <div class="tabs" role="tablist" aria-label={`${label} tabs ${layout.panes.indexOf(item) + 1}`}>
                    {#each item.pane.tabs as id (id)}
                        ...
                        <button class:insert-before={insertBefore === id && dragged !== id} role="tab" id={`${instanceId}-tab-${id}`} aria-selected={item.pane.active === id}
                            ...
                            draggable="true" onclick={() => select(id)} onkeydown={(event) => tabKeys(event, item.pane.id, id)}
                            title="Drag to reorder or move to another pane; Alt+Shift+Left/Right moves between panes"
                            ...
                            <span class:has-short={!!tab?.shortLabel}>{tab?.label}</span>{#if tab?.shortLabel}<span class="short-label">{tab.shortLabel}</span>{/if}
                        </button>
                    {/each}
                </div>
    ...
    {#each tabs as tab (tab.id)}
        ...
        {#if item}
            <div class="panel" class:inactive={item.pane.active !== tab.id} class:drop-target={dropTarget === item.pane.id}
...
<style>
    ...
    [role='tab'] { flex: none; height: 34px; padding: 8px 0 6px; border: 0; border-bottom: 2px solid transparent; color: var(--workspace-muted, var(--rail-inactive, #938371)); font-size: 11px; font-weight: 650; letter-spacing: .08em; text-transform: uppercase; white-space: nowrap; }

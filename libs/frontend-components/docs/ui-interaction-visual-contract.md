...
| Compact harness controls      | Reduce the dev harness below the medium viewport        | Non-active view, Debug, and Admin move from the inline navbar into a menu while retaining their current state and enabled status                                                                                          | The combined active-and-finished Games selector and adjacent new-game action, Player Perspective, actionability, and loaded Game remain unchanged |
...
| Resize the dev harness across the medium breakpoint                  | Developer controls move between the inline navbar and compact menu without changing their values, disabled state, or mutual exclusion                | Returning to the original width restores the original presentation with the same state            | Manual |

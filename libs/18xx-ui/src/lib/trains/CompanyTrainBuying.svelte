<script lang="ts">
    ...
    const companies = $derived.by(() => {
        ...
        for (const choice of session.trainBuying.companyChoices) {
            if (
                choice.source !== session.trainBuying.source ||
                choice.request.seller.kind !== 'company'
            )
                continue
    ...
    {#if response?.asset.kind === 'train' && selectedTrain}
        ...
        >
            ...
            {#if response.seller.kind === 'company'}
                <CompanyToken
                    appearance={session.mapView.stations[response.seller.companyId]}
                    size={24}
                />
            {/if}
    ...
    {:else if request && selectedTrain}
        <div class="summary">
            {#if request.seller.kind === 'company'}
                <CompanyToken
                    appearance={session.mapView.stations[request.seller.companyId]}
                    size={24}
                />
            {/if}
        ...
        <div class="controls">
            <label
                >Price <span>$</span><input
                    aria-label="Train price"
                    type="number"
                    min="1"
                    step="1"
                    value={request.price}
                    oninput={(event) =>
                        session.trainBuying.setCompanyTrainPrice(Number(event.currentTarget.value))}
                /></label
            >
            <button
                class="action-button"
                disabled={!session.decisions.canResolve ||
                    !session.trainBuying.companyEvaluation ||
                    !!session.trainBuying.companyEvaluation.reason}
                onclick={() => session.trainBuying.buyCompanyTrain()}
                    ...
                    <div class="company-roster">
                        {#each trains as choice (choice.request.asset.kind === 'train' ? choice.request.asset.trainId : choice.request.asset.privateCompanyId)}
                            <button
                                ...
                                disabled={!session.decisions.canResolve}
                                onclick={() =>
                                    session.trainBuying.selectCompanyTrain(choice.request)}
                            >
                                <TrainBadge
                                    name={session.trainDepot.trainDefinition(choice.definitionId)
                                        .name}
                                    color={trainColors[choice.definitionId]}
                                />
                            </button>
                        {/each}
                    </div>

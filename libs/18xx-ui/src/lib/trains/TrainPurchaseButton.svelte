...
<style>
    button {
        ...
        min-width: 100px; padding: 4px 8px; border: 1px solid #c7b8a6; border-radius: 4px;
        font: inherit; font-size: 15px; font-weight: 400; line-height: 1.25;
        filter: saturate(0.6); cursor: pointer;
    }
    button > span { font-size: 15px; }
    .price { font-weight: 600; }
    button:disabled { opacity: 0.5; cursor: default; }
    button.upcoming { border-style: dashed; cursor: not-allowed; }
    button:hover:not(:disabled) { filter: saturate(0.6) brightness(0.95); }

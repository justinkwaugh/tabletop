    ...
    <section aria-label="Game ending">
        <div class="results">
            <header>
                ...
                {#if state.finalWealth}<h2>
                        {winners}
                        {state.winningPlayerIds.length > 1 ? 'tie' : 'wins'}
                    </h2>{/if}
                <p class="reason">
                    {state.gameEnding
                        .reason}{#if !state.finalWealth && state.gameEnding.finalOperatingSet !== undefined}
                        · Ends after operating set {state.gameEnding.finalOperatingSet}{/if}
                </p>
            </header>

<script lang="ts">
    ...
    const pillPadding = $derived(
        Math.max(
            0,
            Math.min(
                preferredPadding,
                ((stripWidth - turnActionWidth - 8 - 4) / Math.max(1, selections.length) -
                    labelWidth) /
                    2
            )
        )
    ...
    function measureLabels(element: HTMLElement) {
        ...
        const measure = () => {
            ...
            preferredPadding = parseFloat(
                getComputedStyle(element).getPropertyValue('--preferred-padding')
            )
        }
...
{#if session.gameState.machineState === 'StockRound' && !session.gameState.result}
    <nav aria-label="Stock actions" bind:clientWidth={stripWidth} class:compact>
        {#if selections.length}<div
                class="selections"
                bind:offsetWidth={selectionsWidth}
                style:--pill-padding={`${pillPadding}px`}
                {@attach (node) => {
                    selections
                    return measureLabels(node)
                }}
            >
                <SlidingToggle count={selections.length} {selectedIndex}>
                    {#each selections as selection (selection.label)}<button
                            {disabled}
                            aria-pressed={selection.selected ?? false}
                            onclick={selection.onSelect}><span>{selection.label}</span></button
                        >{/each}
                </SlidingToggle>
            </div>{/if}
...
<style>
    ...
    .selections button:hover:not(:disabled) {
        color: var(--rail-text, #e3e9ef);
    }
    .selections button[aria-pressed='true'] {
        color: #ffffff;
        font-weight: 600;
    }
    .turn-action {

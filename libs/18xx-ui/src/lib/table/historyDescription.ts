...
export function historyDescription(
    ...
    if (isSellShares(action) || isSellFundingShares(action) || isIssueTreasuryShares(action)) {
        ...
        const multipleCompanies = new Set(action.metadata.sales.map((sale) => sale.companyId)).size > 1
        return {
            beforeText: isSellFundingShares(action) && action.metadata.requiredContribution
                ...
                : undefined,
            ledgerText: isIssueTreasuryShares(action) ? `Issued ${action.metadata.sales.map((sale) => `${sale.shares} ${companyName(sale.companyId)}`).join(', ')}` : undefined,
    ...
    if (isContributeTrainFunds(action))
        return { text: `President contributed ${money(action.amount)}`, ledgerText: 'President contributed', important: true }
    ...
    if (isAdvancePhase(action)) {
        ...
        const effects = [
            ...
            ...event.privateEffects
                .filter((effect) => effect.kind === 'close')
                .map((effect) => `${companyName(effect.privateCompanyId)} closed`)
        ]
        return {
    ...
    if (isFloatCompany(action)) {
        ...
        return {
            ...
            detail: capital ? `${money(capital)} capital` : undefined,
            important: true
        }
    }

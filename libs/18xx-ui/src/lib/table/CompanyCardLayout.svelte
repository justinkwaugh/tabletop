<script lang="ts">
    ...
    function observeCards(viewport: HTMLElement, wrap: boolean, count: number) {
        ...
        const grid = element
        const cards = [...grid.children].filter((child): child is HTMLElement => child instanceof HTMLElement)
        function measure() {
            ...
            const minimumColumns = Math.max(1, Math.floor((width + gap) / (cards[0].offsetWidth + gap)))
            for (let candidate = minimumColumns; candidate < count; candidate++) {
                let requiredHeight = 0
                for (let start = 0; start < count; start += candidate) {
                    requiredHeight += Math.max(...cards.slice(start, start + candidate).map(card => card.offsetHeight))
...
<div class="company-card-viewport" {@attach node => observeCards(node, paned, itemCount)}>
    <div class="company-cards" aria-label="Started companies" style:grid-template-columns={`repeat(${columns}, max-content)`}>
        {@render children()}
    </div>
</div>

<style>
    .company-card-viewport { height: 100%; min-height: 0; overflow: auto; }
    .company-cards { display: grid; align-items: start; justify-content: start; gap: 10px; padding: 10px; width: max-content; min-width: 100%; box-sizing: border-box; }
</style>

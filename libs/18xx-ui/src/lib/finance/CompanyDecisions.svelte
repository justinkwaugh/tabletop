<script lang="ts">
    ...
    const purchaseOptions = $derived(
        session.decisions.purchaseOptions.filter(
...
{#if gameState.trackConsent}
    ...
    <DecisionResponse
        label="Track permission response"
        acceptLabel="Allow"
    ...
    <section aria-label="Company decisions" class:private-powers={showPowers}>
        {#if gameState.privatePowerWindow}
            ...
            <button
                ...
                onclick={() => session.decisions.continueOperatingRound()}
                ...
                {#if purchaseOptions.length}
                    <label
                        ...
                        <select
                            ...
                            value=""
                            onchange={(event) => {
                                const choice = purchaseOptions[Number(event.currentTarget.value)]
                            ...
                            <header class="private-track-prompt">
                                ...
                                {#if gameState.privateTrackLay}
                                    ...
                                    <button
                                        ...
                                        disabled={!session.decisions.canResolve ||
                                            !session.validActionTypes.includes(
                                                'DeclinePrivateTile'
                                            )}
                                        onclick={() => session.decisions.declinePrivateTile()}
                                        >skip</button
                                    >
                                {/if}
                            </header>
        ...
        {#if selection && !(selection.kind === 'purchase' && selection.request.asset.kind === 'private')}
            <div aria-label="Company decision preview">
                {#if selection.kind === 'purchase'}
                    ...
                    {:else}<p>
                            The seller’s controlling owner will accept or reject this offer.
                        </p>{/if}
                {:else if selection.kind === 'tile'}
                    ...
                    {#if definition}<Tile
                        ...
                        />{/if}
                    <p>
                        {session.getPlayerName(selection.playerId)} uses {selection.privateCompanyId}:
                        {selection.details.locationId}, cost {selection.details.cost}.
                    </p>
                {:else}<p>

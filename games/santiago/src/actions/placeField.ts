    ...
    implements PlaceField
{
    ...
    apply(state: HydratedSantiagoGameState) {
        ...
        // Only the canal overseer (lowest bidder) is penalised for bidding zero.
        const isOverseer = this.playerId === state.canalOverseerId && state.overseerBidZero
        const farmerCount = isOverseer

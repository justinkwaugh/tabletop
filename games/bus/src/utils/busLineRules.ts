...
export function validBusLineExtensionSegments(line: readonly BusNodeId[]): BusLineSegment[] {
...
export function validBusLineSegments(line: readonly BusNodeId[]): BusLineSegment[] {
    if (line.length === 0) {
        return validStartingBusLineSegments()
    }
    return validBusLineExtensionSegments(line)
}
...
export function extensionSegmentsByTargetNode(
    ...
    for (const segment of validBusLineExtensionSegments(line)) {
...
export function sourceCandidatesForTargetNode(
    ...
    const segments = extensionSegmentsByTargetNode(line).get(targetNodeId) ?? []
...
export function isValidBusLineSegmentPlacement(
    ...
    const [head, tail] = endpoints
    const touchesHead = left === head || right === head
    const touchesTail = left === tail || right === tail

    // Must extend from exactly one endpoint.
    if (touchesHead === touchesTail) {
        return false
    }

    const edgeKey = busLineEdgeKey(left, right)
    return !usedBusLineEdgeKeys(line).has(edgeKey)
}

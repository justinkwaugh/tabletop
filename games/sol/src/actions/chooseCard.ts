...
export type ChooseCardMetadata = Static<typeof ChooseCardMetadata>
export const ChooseCardMetadata = Type.Object({})

export type ChooseCard = Static<typeof ChooseCard>
...
export class HydratedChooseCard extends HydratableAction<typeof ChooseCard> implements ChooseCard {
    ...
    apply(state: HydratedSolGameState, _context?: MachineContext) {
        ...
        if (!(playerState.drawnCards ?? []).find((card) => card.suit === this.suit)) {
            throw Error('Chosen card suit not in drawn cards')
        }

        playerState.card = { suit: this.suit }

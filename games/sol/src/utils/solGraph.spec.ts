...
describe('Sol Graph Tests', () => {
    it('should have no clockwise or counter clockwise neighbors at the center', () => {
        ...
        expect(graph.neighborsAt({ col: 0, row: 0 }, Direction.Clockwise)).toEqual([])
        expect(graph.neighborsAt({ col: 0, row: 0 }, Direction.CounterClockwise)).toEqual([])
    })

    it('can find neighbors in every direction', () => {
        const graph = new SolGraph(4)
        const neighbors = graph.neighborsAt({ row: Ring.Core, col: 0 })
        expect(neighbors.length).toEqual(5)
        const outNeighbors = graph.neighborsAt({ row: Ring.Core, col: 0 }, Direction.Out)
    ...
    it('has the correct node count', () => {
        ...
        const graph5p = new SolGraph(5)
        expect(graph5p.size).toEqual(FIVE_PLAYER_RING_COUNTS.reduce((acc, count) => acc + count, 0))
    })
})

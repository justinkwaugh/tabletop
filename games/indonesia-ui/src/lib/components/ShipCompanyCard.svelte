<script lang="ts">
    ...
    const SHIP_COMPANY_CARD_VIEWBOX_WIDTH = 832.24
    const SHIP_COMPANY_CARD_VIEWBOX_HEIGHT = 551.63
    const SHIP_COMPANY_CARD_VIEWBOX_RX = 67.48
    const SHIP_COMPANY_CARD_VIEWBOX_RY = 67.48
    const SHIP_COMPANY_CARD_ASPECT_RATIO =
        SHIP_COMPANY_CARD_VIEWBOX_WIDTH / SHIP_COMPANY_CARD_VIEWBOX_HEIGHT
    ...
    const width = $derived(height * SHIP_COMPANY_CARD_ASPECT_RATIO)
    ...
    const outlineRx = $derived(
        (width / SHIP_COMPANY_CARD_VIEWBOX_WIDTH) * SHIP_COMPANY_CARD_VIEWBOX_RX
    )
    const outlineRy = $derived(
        (height / SHIP_COMPANY_CARD_VIEWBOX_HEIGHT) * SHIP_COMPANY_CARD_VIEWBOX_RY
    )
...
<g class="pointer-events-none select-none" aria-hidden="true">
    {#if outline}
        ...
        ></rect>
    {/if}
    <ShipCompanyIcon x={iconX} y={iconY} {width} {height} />

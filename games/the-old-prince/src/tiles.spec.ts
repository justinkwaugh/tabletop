...
it('provides unlimited yellow tiles with finite straight and double-dit tiles, including labeled city curves', () => {
    ...
    expect(['7', '8', '9'].map((number) => counts.get(`18xx:${number}`))).toEqual([
        'unlimited',
        'unlimited',
        1
    ])
    ...
    for (const tile of TheOldPrinceTiles.filter((tile) => tile.face.color === 'yellow')) {
        expect(counts.get(tile.id)).toBe(
            tile.id === '18xx:9'
                ? 1
                : tile.face.nodes.filter((node) => node.kind === 'town').length === 2
                  ? 2
                  : 'unlimited'
        )
    }
    for (const label of ['X', 'T']) {
        for (const number of ['5', '6']) {
            const tile = TheOldPrinceTiles.find(
                (tile) => tile.id === `the-old-prince:${number}${label}`
            )

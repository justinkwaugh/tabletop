...
<div class="bg-gray-800/80 rounded-xl p-4 text-white text-center space-y-1 min-w-48">
    {#if machineState === MachineState.Bidding}
        ...
        {#if revealedTiles.length > 0}
            <div class="border-t border-gray-600 pt-2 space-y-1">
                <p class="text-xs text-gray-400 uppercase tracking-wide">This round's tiles</p>
                <div class="flex flex-wrap gap-1">
                    {#each revealedTiles as tile, i}
                        <div class="flex flex-col items-center bg-gray-700 rounded px-2 py-1 text-xs min-w-14">
                            <span class="text-gray-400">#{i + 1}</span>
                            <span class="font-bold capitalize text-white">{tile.crop}</span>
                            <span class="text-gray-300">{tile.farmerCapacity}🧑‍🌾</span>
                        </div>
                    {/each}
                </div>
            </div>
        ...
        {#if placedBids.length > 0}
            <div class="text-xs space-y-0.5 border-t border-gray-600 pt-2 text-left">
                <p class="text-gray-400 uppercase tracking-wide">Bids placed</p>
    ...
    {:else if machineState === MachineState.CanalBuilding}
        {#if isOverseerDecisionPhase}
            ...
            {#if canalProposals.length > 0}
                <div class="text-xs space-y-0.5 border-t border-gray-600 pt-2">
                    <p class="text-gray-400 uppercase tracking-wide">All proposals</p>
                    {#each canalProposals as p}
                        <div class="flex justify-between text-gray-300">
                            <span>{session.game?.players.find(g => g.id === p.playerId)?.name ?? p.playerId}</span>
                            <span class="text-cyan-300">{p.amount} esc</span>
                        </div>
                    {/each}
                </div>
            {:else}
    ...
    {:else if machineState === MachineState.ExtraIrrigation}
        ...
        {#if currentId}
            <p class="text-sm text-gray-300">Waiting for <span class="text-white font-medium">{currentName}</span>…</p>
        {/if}

...
// Every piece of `owner` that can reach one of that owner's castles via a chain of
...
function reachablePieces(
    ...
    for (let r = 0; r < state.board.squares.length; r++) {
        for (let c = 0; c < state.board.squares[r].length; c++) {
            ...
            if (square.knightOwner === owner) pieceKeys.add(key)
            if (square.castleOwner === owner) {
                pieceKeys.add(key)
                castleKeys.push(key)
            }
        }
...
// True if removing the knight at (col,row) would NOT strand any of that owner's
// OTHER knights - i.e. every remaining knight can still reach at least one of that
// owner's castles via a chain of adjacent (non-wall-blocked) knights/castles, the
// same adjacency rule normal knight placement already requires. Removing the last
// knight an owner has is always safe (nothing else depends on it) - per the
...
// stranded knight anywhere made every square of that owner unsafe forever, which quietly
// disabled Renegade against that owner for the rest of the game (ExpandRegion's wall ring
...
export function isKnightSafeToRemove(
    ...
    const after = reachablePieces(state, owner, { col, row })
    ...
    const before = reachablePieces(state, owner)
    for (const key of after.pieceKeys) {

...
import { ActionCard, CardBack } from '../definition/actionCards.js'
...
export const LowenherzGameState = Type.Object({
    ...
    actionDeck: Visibility.protect(Type.Array(ActionCard), { policy: Visibility.Policy.HostOnly }),
    actionDeckBacks: Type.Optional(Type.Array(Type.Enum(CardBack))),
    privateInformation: Type.Optional(Type.Literal(true)),
    finalHands: Type.Optional(
        Type.Array(Type.Object({ playerId: Type.String(), cards: Type.Array(PoliticsCard) }))
    ),
    ...
    politicsCardPileA: Visibility.protect(Type.Array(PoliticsCard), {
        policy: Visibility.Policy.HostOnly
    }),
    politicsPileACount: Type.Optional(Type.Number()),
    politicsCardPileB: Visibility.protect(Type.Array(PoliticsCard), {
        policy: Visibility.Policy.HostOnly
    }),
    politicsPileBCount: Type.Optional(Type.Number()),
...
export const LowenherzProjectedState = Visibility.createProjectionSchema(LowenherzGameState)
export type LowenherzProjectedState = Type.Static<typeof LowenherzProjectedState>
export const LowenherzProjectedStateValidator = Compile(LowenherzProjectedState)

export class HydratedLowenherzGameState
    extends HydratableGameState<typeof LowenherzProjectedState, HydratedLowenherzPlayerState>
    implements LowenherzProjectedState
{
    ...
    declare actionDeckBacks?: CardBack[]
    declare privateInformation?: true
    declare finalHands?: { playerId: string; cards: PoliticsCard[] }[]
    ...
    declare duel?: LowenherzProjectedState['duel']
    ...
    declare politicsPileACount?: number
    declare politicsCardPileB?: PoliticsCard[]
    declare politicsPileBCount?: number
    declare politicsTakingPlayerId?: string
    declare openedPoliticsPile?: 'A' | 'B'

    constructor(data: LowenherzProjectedState) {
        const normalized = normalizeLowenherzState(data)
        super(normalized, LowenherzProjectedStateValidator)

        this.players = normalized.players.map((player) => new HydratedLowenherzPlayerState(player))
    ...
    getActionDeck(): ActionCard[] {
        assertExists(this.actionDeck, 'Action deck is unavailable')
        return this.actionDeck
    }

    getPoliticsPile(pile: 'A' | 'B'): PoliticsCard[] {
        const cards = pile === 'A' ? this.politicsCardPileA : this.politicsCardPileB
        assertExists(cards, 'Politics pile is unavailable')
        return cards
    }

    getPoliticsPileCount(pile: 'A' | 'B'): number {
        const count = pile === 'A' ? this.politicsPileACount : this.politicsPileBCount
        return count ?? this.getPoliticsPile(pile).length
    }

    inspectedPoliticsCards(playerId: string): PoliticsCard[] | undefined {
        const inspection = this.getPlayerState(playerId).politicsInspection
        if (inspection?.pile === this.openedPoliticsPile) return inspection?.cards
        return undefined
    }

...
export enum MachineState {
    ...
    // knight-slot winner hands off to PlacingKnights. A transient state - it never
    // waits for a real player action, always cascading onward via a system action.
    // Region expansion (the other half of the knight action) and politics-card
    // effects aren't built yet - only the winner and any money paid are recorded.

...
describe('HydratedSubmitDuelBid', () => {
    ...
    it('accepts a bid backed by a Treasure card, recording it in metadata', () => {
        ...
        expect(action.metadata).toEqual({
            treasureCardsUsed: [{ type: PoliticsCardType.Treasure, value: 10 }]
        })
    ...
    it('accepts a bid backed by more than one Treasure card - the rulebook does not cap it at one', () => {
        ...
        const action = makeBid('p1', 1, [10, 4])
        expect(action.isValidSubmitDuelBid(state)).toBe(true)
        action.apply(state)

        expect(state.duel!.bids).toEqual([{ playerId: 'p1', amount: 1, treasureValues: [10, 4] }])
        expect(action.metadata).toEqual({
            treasureCardsUsed: [
                { type: PoliticsCardType.Treasure, value: 10 },
                { type: PoliticsCardType.Treasure, value: 4 }
    ...
    it('rejects the same Treasure card listed twice in one bid', () => {
        const state = buildState()
        state.getPlayerState('p1').politicsCards = [{ type: PoliticsCardType.Treasure, value: 10 }]
        expect(makeBid('p1', 1, [10, 10]).isValidSubmitDuelBid(state)).toBe(false)
    })

    it("rejects a Treasure card that is not in the bidder's hand", () => {
        const state = buildState()
        expect(makeBid('p1', 2, [99]).isValidSubmitDuelBid(state)).toBe(false)
    })
    ...
    it('rejects a non-Treasure politics card used as a bid backer', () => {
        ...
        expect(makeBid('p1', 2, [0]).isValidSubmitDuelBid(state)).toBe(false)
    })
    ...
    it("rejects a ducat amount beyond the bidder's money, even with a Treasure card attached", () => {
        ...
        expect(makeBid('p1', 999, [8]).isValidSubmitDuelBid(state)).toBe(false)
    })

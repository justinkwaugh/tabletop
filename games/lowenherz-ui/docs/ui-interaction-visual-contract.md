...
Both are handed over when the deck is clicked, computed from where the chooser's exit slide is aimed rather than measured once it lands, because opening a pile reveals information and is therefore never applied optimistically: the cards exist only after a host round trip, which may return at any point after the click. The chooser holds the clicked deck in its destination slot until the reveal reports something on screen, so the two never both occupy the slot and never both leave it empty. The request is sent before the exit animation, so the round trip runs underneath it.

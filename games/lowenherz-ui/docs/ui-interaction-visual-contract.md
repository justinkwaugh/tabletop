...
- **Break:** one click on the pill submits `CancelAlliance`; the hearts burst. Once the burst has finished and the visible state has settled, the status window announces "[breaker] broke an alliance with [other] and paid 10 ducats." to every viewer until the breaker's turn passes: the breaker's own later actions do not clear it, nor do other participants' duel bids or negotiation moves while the breaker is still active; any other player's action, or the breaker leaving the active set, does.

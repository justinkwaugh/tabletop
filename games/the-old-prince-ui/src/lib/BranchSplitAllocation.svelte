...
{#if preview && allocation}
    <div class="allocation" aria-label="Split asset allocation">
        {#each [false, true] as branch}
            ...
            {@const trains = preview.parentTrains.filter(
                (train) => allocation.trainIds.includes(train.id) === branch
            )}
            {@const destination = branch ? 'parent' : 'branch'}
            <section
                class="panel"
                style:grid-column={branch ? 2 : 1}
                aria-label={branch ? 'Branch assets' : 'Parent assets'}
            >
                <div class="holdings">
                    <header>
                        <CompanyToken appearance={session.mapView.stations[companyId]} size={28} />
                        <strong>{branch ? 'Branch' : 'Parent'}</strong>
                    </header>
                        ...
                        {#each stations as { station, protectedHome } (station.id)}
                            ...
                            <div class="asset" class:branch>
                                ...
                                <button
                                    ...
                                    onclick={() =>
                                        onFocusLocation
                                            ? onFocusLocation(station.position.locationId)
                                            : session.map.inspect({
                                                  kind: 'hex',
                            ...
                            {#each trains as train (train.id)}
                                <div class="asset" class:branch>
                                    <button
                                        ...
                                        onclick={() => session.transferSplitTrain(train.id)}
                                        >{branch ? '←' : '→'}</button
                                    >
                                    <TrainBadge
                                        name={session.trainDepot.trainDefinition(train.definitionId)
                                            .name}
                                        color={TheOldPrinceTrainColors[train.definitionId]}
                                    />
                                </div>
                            {:else}<span class="empty">None</span>{/each}
                        </div>
                ...
                <div class="asset-section cash-section">
                    <h3>Cash</h3>
                    {#if branch}
                        <strong class="cash-total" aria-label="Branch cash"
                            >${(preview.childFunding + allocation.cash).toLocaleString(
                                'en-US'
                            )}</strong
                        >
                    ...
                    {:else}
                        <label
                            >$<input
                                type="number"
                                aria-label="Parent cash"
                                min="0"
                                max={preview.parentCash}
                                step="1"
                                value={preview.parentCash - allocation.cash}
                                disabled={!session.canPreviewSplit}
                                oninput={(event) => {
                                    const value = event.currentTarget.valueAsNumber
                                    if (Number.isFinite(value))
                                        session.setSplitCash(preview.parentCash - value)
                                }}
                            /></label
                        >
                    {/if}
        ...
        <div class="cash-slider">
            ...
            <input
                ...
                disabled={!session.canPreviewSplit || preview.parentCash === 0}
                oninput={(event) => session.setSplitCash(event.currentTarget.valueAsNumber)}
            />
        </div>
    </div>
    {#if session.splitSettlement?.reason}<p class="reason" role="status">
            {!allocation.stationIds.length
                ? 'Allocate at least one branch station'
                : session.splitSettlement.reason}
        </p>{/if}
    <button
        class="confirm"
...
<style>
    ...
    header {
        ...
        gap: 8px;
        padding: 7px 10px;
        background: var(--rail-surface-raised, #eee4d8);
    }
    header strong {
        font-size: 14px;
        font-weight: 600;
    ...
    button:hover:not(:disabled) {
        background: var(--rail-surface-raised, #e6d9c8);
    }
    ...
    input[type='range'] {
        ...
        width: 100%;
        margin: 0;
        accent-color: var(--rail-text, #695543);
        cursor: pointer;
    }
    .reason {
        text-align: center;
    ...
    .confirm:hover:not(:disabled) {
        background: var(--rail-solid, #5b5145);
    }

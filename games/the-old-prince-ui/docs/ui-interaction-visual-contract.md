...
- The published artwork option uses the packaged board image, the Boda Games charter
  ...
  summary, train purchase buttons become the printed train cards, and train and phase badges take
...
| Published tiles | Track construction, published artwork | Pick and lay a tile, then toggle back | Picker, laid tiles, and tiles shown in the action panel and decision prompts use the published tile style with a paper-grain overlay; generic presentation restores the classic style | Toggling back restores the classic tiles | Automated, browser (`boardArtwork.spec.ts`) |
...
| Published trains | Buy trains, published artwork | Open the train buying step | Each purchasable train is its printed card at small size; the price is overlaid only when it differs from the card's printed price (market trains, funding); disabled and upcoming trains dim or show a dashed outline; in History View a purchase reads "Bought" followed by the card, with the price overlaid only when it differs, and clicking the card opens the full-screen mask | Generic presentation restores the coloured buttons | Manual in browser |

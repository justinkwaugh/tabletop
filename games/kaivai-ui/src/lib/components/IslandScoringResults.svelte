<script lang="ts">
    ...
    function totalForPlayer(playerId: string) {
        ...
        const boats = action.metadata?.playerMajorities[playerId].boats ?? 0
        const influence = action.metadata?.playerMajorities[playerId].influence ?? 0
        return huts + boats + influence
    }
</script>
...
<div class="flex flex-row justify-start items-center space-x-2 w-full mt-2">
    ...
    >
        ...
        >
            ...
            <div class="m-auto">
                <img src={influenceImg} alt="influence" class="w-[20px] h-[20px]" />
            </div>
            <div></div>
            <div></div>

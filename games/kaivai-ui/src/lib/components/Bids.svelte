<script lang="ts">
    ...
    let playerIdsByBid = $derived.by(() => {
        const bids: [number, string][] = Object.entries(gameSession.gameState.bids).map(
            ([key, value]) => [value, key]
        )
        return new Map<number, string>(Array.from(bids, (entry) => [entry[0], entry[1]]))
    })
...
<div class="relative">
    <div
        class="rounded-lg bg-[#302408] pt-[8px] pb-[5px] px-2 text-center text-white font-medium flex flex-col justify-between"
    >

...
function buildDockTransferPlanWithContext(
    ...
    // Invariant: boats may only move in reverse during local dock/undock maneuvers.
    // Do not build a dock-to-dock route by reversing an entire plan; reuse the
    // destination dock's local maneuver template and connect it with forward transit.
    const fourPlayerMainToPlayerPlan = buildFourPlayerNoOffshoreMainHarborToOuterPlayerLanePlan(
        startDock,
        endDock,
        plannerContext
    )
    if (fourPlayerMainToPlayerPlan) {
        return fourPlayerMainToPlayerPlan
    }
...
function evaluateDockToOpenWaterPlan(
    ...
    const explicitFourPlayerP1BottomLeftPlan = buildFourPlayerNoOffshoreP1BottomLeftOpenWaterPlan(
        startDock,
        endSlot,
        plannerContext
    )
    if (explicitFourPlayerP1BottomLeftPlan) {
        return explicitFourPlayerP1BottomLeftPlan
    }

    const explicitFourPlayerP1BottomRightPlan =
        buildFourPlayerNoOffshoreP1BottomRightOpenWaterPlan(
            startDock,
            endSlot,
            plannerContext
        )
    if (explicitFourPlayerP1BottomRightPlan) {
        return explicitFourPlayerP1BottomRightPlan
    }

    const explicitFourPlayerP4TopLeftPlan = buildFourPlayerNoOffshoreP4TopLeftOpenWaterPlan(
        startDock,
        endSlot,
        plannerContext
    )
    if (explicitFourPlayerP4TopLeftPlan) {
        return explicitFourPlayerP4TopLeftPlan
    }

    const explicitFourPlayerP4TopRightPlan = buildFourPlayerNoOffshoreP4TopRightOpenWaterPlan(
        startDock,
        endSlot,
        plannerContext
    )
    if (explicitFourPlayerP4TopRightPlan) {
        return explicitFourPlayerP4TopRightPlan
    }
...
function buildFourPlayerNoOffshoreP1BottomLeftOpenWaterPlan(
    startDock: DockSlot,
    endSlot: OpenWaterSlot,
    plannerContext: PlannerContext
): BoatRoutePlan | null {
    if (
        ...
        endSlot.id !== 'open-water-2'
    ) {
        return null
    }

    const laneTargetPose: BoatPose = {
        x: endSlot.stagingPose.x,
        y: endSlot.parkedPose.y,
        heading: Math.PI / 2
    }
    const downwardCandidates = buildUndockCandidates(startDock, laneTargetPose).filter(
        (candidate) => Math.sin(candidate.transitPose.heading) > 0.5
    )
    let bestPlan: BoatRoutePlan | null = null
    ...
    for (const undockCandidate of downwardCandidates) {
        ...
        const radius = endSlot.parkedPose.x - laneStartPose.x
        if (radius < OPEN_WATER_TURN_RADIUS) {
            continue
        }
        ...
        const finalArc = createArcToEndPose(
            endSlot.parkedPose,
            Math.PI / 2,
            normalizeAngle(endSlot.parkedPose.heading - Math.PI / 2),
            radius
        )
        if (
            Math.abs(finalArc.startPose.x - laneStartPose.x) > 2 ||
            finalArc.startPose.y <= laneStartPose.y
        ) {
            continue
        }
        ...
        if (
            !analyzeSegment(verticalSegment, plannerContext).isCollisionFree ||
            !analyzeSegment(finalArc, plannerContext).isCollisionFree
        ) {
            continue
        }
        ...
        const candidatePlan: BoatRoutePlan = {
            undockSegments,
            transitSegments: [verticalSegment],
            dockSegments: [finalArc],
            segments: [...undockSegments, verticalSegment, finalArc]
        }
...
function buildFourPlayerNoOffshoreP1BottomRightOpenWaterPlan(
    startDock: DockSlot,
    endSlot: OpenWaterSlot,
    plannerContext: PlannerContext
): BoatRoutePlan | null {
    if (
        plannerContext.geometry.layoutKey !== '4p-no-offshore' ||
        getPlayerBoardCanonicalSeatId(startDock) !== 'p1' ||
        endSlot.id !== 'open-water-3'
    ) {
        return null
    }

    const bottomChannel = plannerContext.geometry.transitChannels.find(
        (channel) => channel.id === 'channel-bottom-center'
    )
    const bottomChannelY =
        bottomChannel ? bottomChannel.y - 40 : plannerContext.geometry.boardHeight - 170
    const coneEntryHeading = (11 * Math.PI) / 36
    const finalApproachRadius = OPEN_WATER_TURN_RADIUS * 1.2
    const coneTurnFactor = 1 - Math.cos(Math.abs(coneEntryHeading))
    const coneTurnRadius =
        (endSlot.parkedPose.y - bottomChannelY) / coneTurnFactor - finalApproachRadius
    if (coneTurnRadius < OPEN_WATER_TURN_RADIUS) {
        return null
    }

    const maneuverConfig = DOCK_MANEUVER_CONFIG[startDock.family]
    const clearancePose: BoatPose = {
        x:
            startDock.stagingPose.x -
            Math.cos(startDock.stagingPose.heading) * maneuverConfig.undockReverseDistance,
        y:
            startDock.stagingPose.y -
            Math.sin(startDock.stagingPose.heading) * maneuverConfig.undockReverseDistance,
        heading: startDock.stagingPose.heading
    }
    const reverseSegment = createStraightSegment(startDock.stagingPose, clearancePose, 'reverse')
    const reverseTurnSegment = createReverseArcFromStartPose(
        clearancePose,
        normalizeAngle(Math.PI / 2 - clearancePose.heading),
        maneuverConfig.undockReverseTurnRadius
    )
    const laneStartPose = reverseTurnSegment.endPose
    if (Math.abs(normalizeAngle(laneStartPose.heading - Math.PI / 2)) > 0.02) {
        return null
    }

    const turnToBottomChannelRadius = OPEN_WATER_TURN_RADIUS
    const coneTurnStartX =
        endSlot.parkedPose.x -
        Math.abs(Math.sin(coneEntryHeading)) * (coneTurnRadius + finalApproachRadius)
    const verticalEndPose: BoatPose = {
        x: laneStartPose.x,
        y: bottomChannelY - turnToBottomChannelRadius,
        heading: Math.PI / 2
    }
    if (verticalEndPose.y <= laneStartPose.y) {
        return null
    }
    ...
    const bottomChannelArc = createForwardArcFromStartPose(
        verticalEndPose,
        -Math.PI / 2,
        turnToBottomChannelRadius
    )
    if (
        Math.abs(bottomChannelArc.endPose.y - bottomChannelY) > 2 ||
        Math.abs(normalizeAngle(bottomChannelArc.endPose.heading)) > 0.02
    ) {
        return null
    }

    const bottomChannelHandoffPose: BoatPose = {
        x: coneTurnStartX,
        y: bottomChannelArc.endPose.y,
        heading: 0
    }
    if (bottomChannelHandoffPose.x <= bottomChannelArc.endPose.x + 48) {
        return null
    }

    const bottomChannelSegment = createStraightSegment(
        bottomChannelArc.endPose,
        bottomChannelHandoffPose,
        'forward'
    )
    const coneEntryArc = createForwardArcFromStartPose(
        bottomChannelHandoffPose,
        coneEntryHeading,
        coneTurnRadius
    )
    const finalArc = createArcToEndPose(
        endSlot.parkedPose,
        coneEntryHeading,
        normalizeAngle(endSlot.parkedPose.heading - coneEntryHeading),
        finalApproachRadius
    )
    if (
        distanceBetween(coneEntryArc.endPose, finalArc.startPose) > 2 ||
        Math.abs(normalizeAngle(coneEntryArc.endPose.heading - finalArc.startPose.heading)) > 0.02
    ) {
        return null
    }
    ...
    if (
        !analyzeSegment(verticalSegment, plannerContext).isCollisionFree ||
        !analyzeSegment(bottomChannelArc, plannerContext).isCollisionFree ||
        !analyzeSegment(bottomChannelSegment, plannerContext).isCollisionFree ||
        !analyzeSegment(coneEntryArc, plannerContext).isCollisionFree ||
        !analyzeSegment(finalArc, plannerContext).isCollisionFree
    ) {
        return null
    }
    ...
    return {
        undockSegments,
        transitSegments: [verticalSegment, bottomChannelArc, bottomChannelSegment, coneEntryArc],
        dockSegments: [finalArc],
        segments: [
            ...undockSegments,
            verticalSegment,
            bottomChannelArc,
            bottomChannelSegment,
            coneEntryArc,
            finalArc
        ]
    }
...
function buildFourPlayerNoOffshoreP4TopLeftOpenWaterPlan(
    startDock: DockSlot,
    endSlot: OpenWaterSlot,
    plannerContext: PlannerContext
): BoatRoutePlan | null {
    ...
    const upwardCandidates = buildUndockCandidates(startDock, laneTargetPose).filter(
        (candidate) => Math.sin(candidate.transitPose.heading) < -0.5
    )
    let bestPlan: BoatRoutePlan | null = null
...
function buildFourPlayerNoOffshoreP4TopRightOpenWaterPlan(
    startDock: DockSlot,
    endSlot: OpenWaterSlot,
    plannerContext: PlannerContext
): BoatRoutePlan | null {
    if (
        plannerContext.geometry.layoutKey !== '4p-no-offshore' ||
        getPlayerBoardCanonicalSeatId(startDock) !== 'p4' ||
        endSlot.id !== 'open-water-1'
    ) {
        return null
    }

    const topChannel = plannerContext.geometry.transitChannels.find(
        (channel) => channel.id === 'channel-top-center'
    )
    const topChannelY = topChannel ? topChannel.y + 40 : FOUR_PLAYER_TOP_HARBOR_CHANNEL_Y - 50
    const coneEntryHeading = (-11 * Math.PI) / 36
    const finalApproachRadius = OPEN_WATER_TURN_RADIUS * 1.2
    const coneTurnFactor = 1 - Math.cos(Math.abs(coneEntryHeading))
    const coneTurnRadius =
        (topChannelY - endSlot.parkedPose.y) / coneTurnFactor - finalApproachRadius
    if (coneTurnRadius < OPEN_WATER_TURN_RADIUS) {
        return null
    }
    ...
    const turnToTopChannelRadius = OPEN_WATER_TURN_RADIUS
    const coneTurnStartX =
        endSlot.parkedPose.x -
        Math.abs(Math.sin(coneEntryHeading)) * (coneTurnRadius + finalApproachRadius)
    const verticalEndPose: BoatPose = {
        x: laneStartPose.x,
        y: topChannelY + turnToTopChannelRadius,
        heading: -Math.PI / 2
    }
    if (verticalEndPose.y >= laneStartPose.y) {
        return null
    ...
    const topChannelArc = createForwardArcFromStartPose(
        verticalEndPose,
        Math.PI / 2,
        turnToTopChannelRadius
    )
    if (
        Math.abs(topChannelArc.endPose.y - topChannelY) > 2 ||
        Math.abs(normalizeAngle(topChannelArc.endPose.heading)) > 0.02
    ) {
        return null
    }

    const topChannelHandoffPose: BoatPose = {
        x: coneTurnStartX,
        y: topChannelArc.endPose.y,
        heading: 0
    }
    if (topChannelHandoffPose.x <= topChannelArc.endPose.x + 48) {
        return null
    }

    const topChannelSegment = createStraightSegment(
        topChannelArc.endPose,
        topChannelHandoffPose,
        'forward'
    )
    const coneEntryArc = createForwardArcFromStartPose(
        topChannelHandoffPose,
    ...
    if (
        !analyzeSegment(verticalSegment, plannerContext).isCollisionFree ||
        !analyzeSegment(topChannelArc, plannerContext).isCollisionFree ||
        !analyzeSegment(topChannelSegment, plannerContext).isCollisionFree ||
        !analyzeSegment(coneEntryArc, plannerContext).isCollisionFree ||
        !analyzeSegment(finalArc, plannerContext).isCollisionFree
    ) {
        return null
    ...
    return {
        undockSegments,
        transitSegments: [verticalSegment, topChannelArc, topChannelSegment, coneEntryArc],
        dockSegments: [finalArc],
        segments: [
            ...undockSegments,
            verticalSegment,
...
function buildFourPlayerNoOffshoreP1MainHarborLanePlan(
    startDock: DockSlot,
    endDock: DockSlot,
    plannerContext: PlannerContext
): BoatRoutePlan | null {
    ...
    const upwardCandidates = buildUndockCandidates(startDock, undockTargetPose).filter(
        (candidate) => Math.sin(candidate.transitPose.heading) < -0.35
    )
    if (upwardCandidates.length === 0) {
        return null
    ...
    for (const undockCandidate of upwardCandidates) {
        const preLaneTransitSegments: BoatMotionSegment[] = []
        let currentPose = undockCandidate.transitPose

        const desiredVerticalHeading = -Math.PI / 2
        const topLaneEntryY = topCenterChannel.y + FOUR_PLAYER_MAIN_HARBOR_ARC_RADIUS
        ...
        if (Math.abs(alignmentDelta) > Math.PI / 24) {
            ...
            preLaneTransitSegments.push(alignmentArc)
        ...
        if (
            !analyzeSegment(toLaneVertical, plannerContext).isCollisionFree ||
            !analyzeSegment(arcToHorizontal, plannerContext).isCollisionFree
        ) {
            continue
        }
...
function buildFourPlayerNoOffshoreP4MainHarborLanePlan(
    startDock: DockSlot,
    endDock: DockSlot,
    plannerContext: PlannerContext
): BoatRoutePlan | null {
    ...
    const downwardCandidates = buildUndockCandidates(startDock, undockTargetPose).filter(
        (candidate) => Math.sin(candidate.transitPose.heading) > 0.35
    )
    if (downwardCandidates.length === 0) {
        return null
    ...
    for (const undockCandidate of downwardCandidates) {
        ...
        const desiredVerticalHeading = Math.PI / 2
        const bottomLaneEntryY = bottomCenterChannel.y - FOUR_PLAYER_MAIN_HARBOR_ARC_RADIUS
        const alignmentDelta = normalizeAngle(desiredVerticalHeading - currentPose.heading)
        if (Math.abs(alignmentDelta) > Math.PI / 24) {
            const alignmentArc = createForwardArcFromStartPose(
                currentPose,
...
function buildFourPlayerNoOffshoreP2MainHarborLanePlan(
    startDock: DockSlot,
    endDock: DockSlot,
    plannerContext: PlannerContext
): BoatRoutePlan | null {
    if (
        plannerContext.geometry.layoutKey !== '4p-no-offshore' ||
        getPlayerBoardCanonicalSeatId(startDock) !== 'p2' ||
        endDock.family !== 'main-island-harbor'
    ) {
        return null
    }
    ...
    const targetLaneX = Math.max(...dockCandidates.map((candidate) => candidate.transitPose.x))
    const downwardCandidates = buildUndockCandidates(startDock, {
        x: targetLaneX,
        y: plannerContext.geometry.boardHeight,
        heading: desiredSetupHeading
    }).filter((candidate) => Math.sin(candidate.transitPose.heading) > 0.35)
    if (downwardCandidates.length === 0) {
        return null
    }
    ...
    for (const undockCandidate of downwardCandidates) {
        ...
        for (const dockCandidate of dockCandidates) {
            ...
            if (overshootPose.y <= currentPose.y + 12) {
                continue
            }

            const southboundPassBy = createStraightSegment(currentPose, overshootPose, 'forward')
            if (!analyzeSegment(southboundPassBy, plannerContext).isCollisionFree) {
                continue
            }
            ...
            const candidatePlan: BoatRoutePlan = {
                undockSegments,
                transitSegments: [...transitSegments, southboundPassBy],
                dockSegments,
                segments: [...undockSegments, ...transitSegments, southboundPassBy, ...dockSegments]
            }
...
function buildFourPlayerNoOffshoreP3MainHarborLanePlan(
    startDock: DockSlot,
    endDock: DockSlot,
    plannerContext: PlannerContext
): BoatRoutePlan | null {
    ...
    const targetLaneX = Math.max(...dockCandidates.map((candidate) => candidate.transitPose.x))
    const upwardCandidates = buildUndockCandidates(startDock, {
        x: targetLaneX,
        y: 0,
        heading: desiredSetupHeading
    }).filter((candidate) => Math.sin(candidate.transitPose.heading) < -0.35)
    ...
    for (const undockCandidate of upwardCandidates) {
        ...
        for (const dockCandidate of dockCandidates) {
            ...
            const northboundPassBy = createStraightSegment(currentPose, overshootPose, 'forward')
            if (!analyzeSegment(northboundPassBy, plannerContext).isCollisionFree) {
                continue
            }
            ...
            const candidatePlan: BoatRoutePlan = {
                undockSegments,
                transitSegments: [...transitSegments, northboundPassBy],
                dockSegments,
                segments: [...undockSegments, ...transitSegments, northboundPassBy, ...dockSegments]
            }
...
function buildFourPlayerNoOffshoreMainHarborLanePlan(
    ...
    const startSeat = getPlayerBoardCanonicalSeatId(startDock)
    if (startSeat === 'p1') {
        return buildFourPlayerNoOffshoreP1MainHarborLanePlan(
            startDock,
            endDock,
            plannerContext
        )
    ...
    if (startSeat === 'p2') {
        return buildFourPlayerNoOffshoreP2MainHarborLanePlan(
            startDock,
            endDock,
            plannerContext
        )
    ...
    if (startSeat === 'p4') {
        return buildFourPlayerNoOffshoreP4MainHarborLanePlan(
            startDock,
            endDock,
            plannerContext
        )
    ...
    if (startSeat === 'p3') {
        return buildFourPlayerNoOffshoreP3MainHarborLanePlan(
            startDock,
            endDock,
            plannerContext
        )
...
function buildFourPlayerNoOffshoreMainHarborToOuterPlayerLanePlan(
    startDock: DockSlot,
    endDock: DockSlot,
    plannerContext: PlannerContext
): BoatRoutePlan | null {
    ...
    const topCenterChannel = transitChannelsById.get('channel-top-center')
    const bottomCenterChannel = transitChannelsById.get('channel-bottom-center')
    if (!topCenterChannel || !bottomCenterChannel) {
        return null
    }

    const endSeat = getPlayerBoardCanonicalSeatId(endDock)
    if (endSeat === 'p1') {
        return buildFourPlayerNoOffshoreMainHarborToP1LanePlan(
            startDock,
            endDock,
            plannerContext,
        )
...
function buildFourPlayerNoOffshoreMainHarborToP1LanePlan(
    startDock: DockSlot,
    endDock: DockSlot,
    plannerContext: PlannerContext
): BoatRoutePlan | null {
...
function buildFourPlayerNoOffshoreMainHarborToP4LanePlan(
    startDock: DockSlot,
    endDock: DockSlot,
    plannerContext: PlannerContext
): BoatRoutePlan | null {
...
function buildStructuredChannelDockTransferPlan(
    ...
    const outerPlayerLanePlan = buildFourPlayerNoOffshoreMainHarborToOuterPlayerLanePlan(
        startDock,
        endDock,
        plannerContext
    )
    if (outerPlayerLanePlan) {
        return outerPlayerLanePlan
    }
...
function buildForcedChannelDockTransferPlan(
    ...
    options: {
        finalDockCandidateOptions?: DockCandidateOptions
    } = {}
): BoatRoutePlan | null {
    let bestPlan: BoatRoutePlan | null = null
    ...
    for (const undockCandidate of undockCandidates) {
        const dockConnection = findExactDockConnectionViaChannelChain(
            ...
            plannerContext,
            channels,
            options.finalDockCandidateOptions
        )
        if (!dockConnection) continue
...
function findExactDockConnectionDirect(
    ...
    endDock: DockSlot,
    plannerContext: PlannerContext,
    dockCandidateOptions: DockCandidateOptions = {}
): { transitSegments: BoatMotionSegment[]; dockSegments: BoatMotionSegment[]; endDock: DockSlot } | null {
    const shouldPreferHarborKTurn =
        plannerContext.geometry.layoutKey !== '3p-no-offshore' &&
        endDock.family === 'main-island-harbor'
    const standardGoals = buildDockCandidates(endDock, startPose, {
        ...dockCandidateOptions,
...
function findExactDockConnectionViaChannelChain(
    ...
    const finalConnection = findExactDockConnectionDirect(
        currentPose,
        endDock,
        plannerContext,
        dockCandidateOptions
...
type DockCandidateOptions = {
    ...
    obliqueAngleOffsets?: readonly number[]
}

function buildDockCandidates(
    dock: DockSlot,
    sourcePose: BoatPose,
    options: DockCandidateOptions = {}
...
const PLAYER_BOARD_APPROACH_ANGLE_OFFSETS = [Math.PI / 9, Math.PI / 6, Math.PI / 4, Math.PI / 3]
...
function buildObliqueArcGoalCandidates(
    ...
    preferredBearing: number,
    scoreBase: number,
    angleOffsets: readonly number[] = OBLIQUE_APPROACH_ANGLE_OFFSETS
): DockManeuverCandidate[] {
...
function reverseMotionSegment(segment: BoatMotionSegment): BoatMotionSegment {
    const reversedDirection = segment.direction === 'forward' ? 'reverse' : 'forward'

    if (segment.kind === 'straight') {
        return createStraightSegment(segment.endPose, segment.startPose, reversedDirection)
    }
    ...
    if (segment.kind === 'pivot') {
        return {
            ...
            startPose: segment.endPose,
            endPose: segment.startPose,
    ...
    return createArcSegment({
        ...
        pivotOffsetAlongHeading: segment.pivotOffsetAlongHeading
    })
}

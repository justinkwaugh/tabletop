...
The [implementation review](hidden-information-review.md) records the 2026-09-04 correctness findings, developer-experience limitations, migration decisions, and remaining work. The [slice-6 compatibility audit](hidden-information-compatibility.md) records subsequent fixes, current validation, the publication matrix, and the next release-critical decisions. Passing scenario tests do not mean the remaining release requirements have been resolved.
...
Hosted and Hotseat Forks copy the real canonical position through the shared `createGameFork` operation, independently of visibility registration. The host always loads canonical current state and history; Hotseat and the development harness use their local canonical data. Historical positions are reconstructed by reversing only the suffix after the requested position. Already-triggered System Actions are included through the next player decision. No setup or historical rules are executed, and both randomness streams and the original system version are preserved.

Inherited Action IDs and state identity remain intact within the new Game's storage scope, preserving title-owned references and checksums. Game IDs in inherited patches are normalized to the new Game. Current-position Forks do not depend on old patches or Action schemas. Historical reconstruction or current-schema hydration failure returns an explicit cannot-fork error before persistence. Hosted Forks preserve the existing owner, reservation, joining, and start lifecycle. Their state, Game metadata, and Action chunks are written in one transaction. See [Canonical Forks](hidden-information-forks.md) for the implementation and validation boundary.

...
This direct request/response slice is not an end-to-end confidentiality boundary. `AddActions` Realtime Updates are still broadcast from canonical records, incremental synchronization and Hosted Undo still return canonical records, and other surfaces such as validation errors remain unprojected. A participating title therefore must not be treated as safe to deploy for hidden information until those paths are closed. The direct Action path uses a full projected reload for its own error recovery specifically to avoid falling back to the canonical synchronization response.
...
> **Implementation experiment status:** This remains a working proposal, not an accepted direction. Schema declaration and derivation, pure state and Action projection, optional Game Runtime projector registration, canonical Action-cascade capture, visible patch materialization, on-demand full-history materialization from persistence, ordinary initial-load transport, and the direct Action-submission response plus authoritative client application now have initial implementations. Projected hydration, replay classification, Realtime Update, synchronization, undo delivery, and the Developer/Admin Host Game Context remain design work.

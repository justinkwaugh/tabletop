...
> **Implementation experiment status:** This remains a working proposal, not an accepted direction. Schema declaration and derivation, pure state and Action projection, optional Game Runtime projector registration, canonical Action-cascade capture, visible patch materialization, on-demand full- and partial-history materialization from persistence, ordinary initial-load transport, the direct Action-submission response plus authoritative client application, additive per-Perspective Realtime Action delivery, and incremental synchronization now have initial implementations. Projected hydration, replay classification, undo delivery, other potentially state-bearing notification flows, and the Developer/Admin Host Game Context remain design work.

...
Exploration sources are advanced privately to the end of the selected cascade before population; closing restores the original History position or returns to the latest Live state. Recorded source History remains navigable within its supported range. Play uses the separate sampled state; inherited Undo stops at the first forward-patched or non-optimistically executable cascade. Saved checkpoints retain reversible source/hypothetical difference patches, preserving the sample without two full state copies or resampling on reload. Authorized Debug/Admin Host View Exploration uses `runtime.exploration.createFromCanonicalState` when supplied, without requiring projected population. Titles without state-specific preparation use the copied canonical state directly. See [Exploration](hidden-information-exploration.md).

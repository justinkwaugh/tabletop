...
Existing Game notification schemas remain unchanged, including the optional `Game.state` on every legacy payload, so a current consumer can still accept a queued or older state-bearing message. New outbound `NotificationCategory.Game` notifications all pass through one constructor that removes `Game.state`; the additive `addProjectedActions` and `replaceProjectedActions` schemas additionally require a state-free Game. This changes no discriminator or required field for older consumers because state was already optional. In particular, starting a Game stores the canonical initial state but returns only state-free Game metadata to the initiating Site Frontend and emits a state-free Update; navigation then performs the ordinary perspective-projected initial load. The Game seed likewise remains present in Game metadata; whether it or PRNG state must be projected is deliberately deferred.
...
Realtime Undo uses the additive `replaceProjectedActions` discriminator. As with live Action publication, the host sends one spectator replacement on the shared Game Instance topic and one Player replacement on each associated User topic. A current Game Session accepts only its own Perspective and reconciles directly from the supplied complete projected suffix, including records not already present locally. The requester ignores its own Player update when it arrives while the direct call is busy and applies the direct response instead; a later duplicate is still a complete replacement and converges to the same checksum. A non-participating Game retains the exact shared `undoAction` notification with its compact canonical ID manifest, and older Game Sessions ignore the new discriminator. This preserves the old protocol without asking a projected client to recover missing records from canonical data.
...
A non-hotseat remote Game whose runtime registers visibility chooses submission handling from `runtime.visibility.optimisticActionTypes`. An approved Action uses the existing Optimistic Application path, which executes the initiating User Action and generated System Action cascade locally before awaiting persistence. An Action not on that list waits for the host and applies every supplied Processed Action exactly once through `GameEngine.applyProcessedAction`. The host independently classifies the projected result: a proven replayable cascade omits every `forwardPatch`, while a failed probe retains a patch on every record. If an optimistically executed submission receives a patched cascade or a different authoritative Action trace, the client restores its prior projected context and applies the supplied records instead of retaining the optimistic result. It then verifies the complete Action checksum. A failed direct Action response or mismatch still conservatively restores prior projected data and synchronizes.

Hosted Undo remains authoritative for a participating remote Game, preventing local game logic from running against redacted state before the projected replacement arrives. Games without a visibility registration, hotseat Games, local Games, and Exploration contexts retain their existing local execution path. Exploration created from projected state may be unable to simulate rules that require unknown canonical values; that is a separate unresolved lifecycle design.

These live paths are not yet an end-to-end confidentiality boundary. Initial load, direct Action results, Realtime Action delivery, incremental synchronization, and direct and Realtime Hosted Undo are projected, while generic Game notifications and the start response are state-free. Other surfaces such as legal-Action discovery and validation errors remain unprojected. A participating title therefore must not be treated as complete hidden-information support until those paths and the explicitly accepted transport-authorization limitation are addressed for its threat model.
...
> **Implementation experiment status:** This remains a working proposal, not an accepted direction. Schema declaration and derivation, pure state and Action projection, optional Game Runtime projector registration, canonical Action-cascade capture, visible patch materialization, automatic whole-cascade replay classification, explicit optimistic-Action registration, on-demand full- and partial-history materialization from persistence, ordinary initial-load transport, direct Action-submission and Hosted Undo responses plus client application, additive per-Perspective Realtime Action and Undo delivery, and incremental synchronization now have initial implementations. Other potentially state-bearing notification flows and the Developer/Admin Host Game Context remain design work.
...
Optimistic execution remains a separate declaration because a client must decide whether to execute before the host has produced a result that can be probed. `runtime.visibility.optimisticActionTypes` explicitly approves that smaller set. Fresh Fish currently approves `PlaceDisk`, `PlaceMarket`, `PlaceStall`, and `Pass`; `DrawTile` and `PlaceBid` wait for the host. A patched response or differing Action trace forces an optimistic client to roll back and apply the authoritative visible records.
...
- a participating non-hotseat remote Game Client optimistically executes only Action types explicitly approved by its visibility registration, waits for all other submissions and Hosted Undo, applies replayable or patched host traces exactly once, rolls an optimistic result back when the host returns patches or a different trace, verifies the full checksum, and synchronizes after errors;
...
- Fresh Fish client tests prove that `PlaceDisk` updates projected state before a delayed host response, both replayable and patched responses reconcile to the same host projection, and `DrawTile` leaves local state untouched until its delayed host response arrives;
...
- How should the initial `runtime.visibility.state` and `runtime.visibility.actions` Interface evolve when transition-aware policies and opaque Action types are implemented?
...
- How should an Exploration based on a projected context model unknown canonical values without revealing them or becoming unable to execute secret-dependent rules?

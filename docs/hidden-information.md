...
### Realtime projected Action delivery and compatibility
...
The distinct discriminator is the mixed-artifact compatibility boundary. An older Game Session continues to recognize `addActions` and ignores the unknown `addProjectedActions` action rather than accidentally applying both the spectator and Player projections. A Game Title must therefore publish a matching UI Artifact before opting its Logic Artifact into visibility; this is the same opt-in publication constraint as projected initial load and Action responses. Titles that do not opt in continue receiving the legacy protocol without republishing their UI Artifacts. The Site Frontend's generic notification envelope already permits additive action strings, so this does not change the injected Notification Service interface.

Existing Game notification schemas remain unchanged, including the optional `Game.state` on every legacy payload. The new `addProjectedActions` schema alone requires a state-free Game. This preserves the existing contract for deployed consumers, but also means that existing state-bearing notifications—most notably the Update emitted when a Game starts—remain a confidentiality surface that must receive a separate compatibility-safe design before hidden information is deployable end to end. The Game seed likewise remains present in Game metadata; whether it or PRNG state must be projected is deliberately deferred.

Private delivery also requires transport authorization, not merely a user-specific or hard-to-guess topic name. The current Ably token grants broad channel capabilities, and [Ably capability wildcards operate on colon-delimited namespace segments](https://ably.com/docs/auth/capabilities), so they cannot select the existing hyphenated `game-{id}` channel convention. A draft restriction using `game-*` was rejected because Ably treats that as a literal channel rather than a prefix match, which would have broken every deployed Game subscription without protecting the intended channels. Before Player projections can be treated as confidential in production, the transport needs a compatibility-safe authorization design, such as authorized exact-channel grants or a staged move to colon-delimited namespaces. This experiment deliberately leaves existing Ably credentials unchanged until that migration is designed.
...
As the first conservative live path, a non-hotseat remote Game whose runtime registers visibility does not Optimistically Apply a submitted Action. Its Game Session sends the Unprocessed Action with the local Action count as its proposed index, combines the response's projected missing suffix and newly processed Action trace in canonical index order, and applies every supplied record exactly once through `GameEngine.applyProcessedAction`. The current projector puts a `forwardPatch` on every record, so this path does not hydrate an Action, run game rules against redacted state, regenerate System Actions, or consume client PRNG state. It then installs the returned Game metadata and verifies the complete Action checksum. A failed response or mismatch restores the prior projected context and performs an ordinary full projected reload rather than calling the still-canonical incremental synchronization path.

This is deliberately broader than the eventual need: all Actions of a participating remote Game currently wait for the server, even though most may later prove safe for Optimistic Application. Games without a visibility registration, hotseat Games, local Games, and Exploration contexts retain their existing local execution path. Exploration created from projected state may be unable to simulate rules that require unknown canonical values; that is a separate unresolved lifecycle design.

These live Action paths are not yet an end-to-end confidentiality boundary. Realtime Action delivery is projected, but incremental synchronization and Hosted Undo still return canonical records, existing state-bearing Game notifications remain unprojected, and other surfaces such as validation errors remain unprojected. A participating title therefore must not be treated as safe to deploy for hidden information until those paths are closed. The direct Action path uses a full projected reload for its own error recovery specifically to avoid falling back to the canonical synchronization response.
...
> **Implementation experiment status:** This remains a working proposal, not an accepted direction. Schema declaration and derivation, pure state and Action projection, optional Game Runtime projector registration, canonical Action-cascade capture, visible patch materialization, on-demand full-history materialization from persistence, ordinary initial-load transport, the direct Action-submission response plus authoritative client application, and additive per-Perspective Realtime Action delivery now have initial implementations. Projected hydration, replay classification, synchronization, undo delivery, other potentially state-bearing notification flows, and the Developer/Admin Host Game Context remain design work.
...
- a participating non-hotseat remote Game Client waits for that authoritative response, applies its complete projected trace exactly once through safe forward patches, verifies the full checksum, and uses a complete projected reload for error recovery;
...
- What additive notification contracts should project state-bearing Game Updates and Hosted Undo without changing the meaning of messages consumed by older Game Sessions?
- How should Realtime transport credentials authorize only the current User's projected topic while preserving existing Game subscriptions during a channel-namespace migration?

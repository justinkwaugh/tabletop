...
For a new Table with `p` Players, creation uses `p + 3` transactional reads: Game existence, Tournament, Schedule and the `p` accounts. It writes two Game documents, an optional private-seed document, and the Tournament only for its first Game. This excludes transaction retries, initial service cache misses, the post-transition Tournament cache refill, and ordinary notification storage. No per-Table document or schedule rewrite is added. An acknowledged retry served from cache does no Game writes or reinitialization; the Tournament revision notification may be repeated. Durable notification/task recovery belongs to slice 6; current notification delivery retains the site's existing best-effort behavior.

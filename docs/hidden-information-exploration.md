...
- Undo changes the playable hypothetical state. For projected-source Exploration, inherited Undo requires that reversing the complete User/System cascade reconstructs complete state from which canonical execution reproduces the hypothetical result and Action identities. Stop at the first cascade that cannot establish this. Patch presence and optimistic-execution flags do not determine authoritative execution eligibility. Redacted Actions cannot be executed as complete Actions.
- Canonical-source Exploration can undo the complete recorded history, including revealing Actions. At the source boundary, restore the recorded canonical state before applying older undo patches, then prepare fresh hypothetical state at the earlier position. Never apply original patches to a shuffled hypothetical deck. A branch created from an existing Exploration retains its inherited projected Undo limit.
...
Determine inherited Undo eligibility by verifying canonical execution from the reconstructed hypothetical state. Safely reversible source Actions may be undone; unsafe Actions remain available for recorded History navigation. Preserve hypothetical state and randomness when reversing the safe suffix. Verify actual reconstruction rather than inferring execution eligibility from patch presence.
...
The framework removes the source master seed, independently initializes the branch's public and version-3 protected PRNGs, and gives population fresh sampling randomness. Runtimes declaring `randomnessVersion: 1` use fresh ChaCha20 protected entropy for the hypothetical future; legacy runtimes retain numeric SplitMix32 cursors. The optional `explorationState.checkpoint` stores two directional difference patches: `source` converts the hypothetical boundary state back to the permitted source, and `hypothetical` converts that source into the sampled state. Both exclude recursive Exploration metadata. Unchanged state is not copied into the checkpoint. Existing Action storage retains the source prefix plus simulated suffix; there is no source-world reconstruction from the protected seed.
...
### Canonical-source Undo

New checkpoints optionally record `canonicalSource: true`. Its absence preserves the existing projected/saved checkpoint behavior. Canonical inherited Undo rebases both boundary patches after fresh state preparation; Undo confined to exploration-generated Actions preserves the existing sample and random cursors. Historical states still must reconstruct and validate under the current runtime for Undo to succeed.

This change is bundled into Game UI Artifacts. Republish Fresh Fish UI for the auction selection and canonical Exploration fixes, and other title UIs to adopt the shared Exploration correction. No backend, Site Frontend bridge, or Logic Artifact change is required. Previously saved checkpoints keep their prior Undo limits; create a new branch to obtain canonical-source Undo. New canonical checkpoints require the updated UI implementation for safe inherited Undo; older UI Artifacts do not interpret the marker.

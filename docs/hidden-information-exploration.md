...
- Undo changes the playable hypothetical state. For projected-source Exploration, when undoing inherited Actions, stop before the first Action with a forward patch or requiring non-optimistic execution. Check the entire User/System cascade; do not skip an unsafe System Action. Existing protected/sentinel Actions also cannot be executed as complete Actions.

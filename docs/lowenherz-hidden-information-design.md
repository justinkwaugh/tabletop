...
Turning off `publicMoney` now omits other players’ balances from protected delivery during play. Owners retain their balance; EndOfGame reveals all balances. Missing configuration defaults to public money. The create-game toggle presents this option inverted, as “Private money”, and stores `publicMoney: false` when switched on. Public starting balances and transactions still permit accounting; concealment removes the running total, not those observations. The state projector and execution guard both receive Game configuration, and rules use asserted money accessors so an unavailable balance cannot become zero or NaN.

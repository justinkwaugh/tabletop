...
Enabling `privateMoney` now omits other players’ balances from protected delivery during play. Owners retain their balance; EndOfGame reveals all balances. Missing configuration defaults to public money. Public starting balances and transactions still permit accounting; concealment removes the running total, not those observations. The state projector and execution guard both receive Game configuration, and rules use asserted money accessors so an unavailable balance cannot become zero or NaN.
...
The private-money follow-up passes 341 Löwenherz tests and the Logic type check, UI build, and UI check (zero errors, six existing warnings). A local browser smoke check verified hidden balances and disabled Exploration in hotseat, protected player, spectator, and Host View, with no browser errors. This follow-up has not been deployed.


## Configuration compatibility

Löwenherz configuration uses `privateMoney`, `standardSetup`, and `allowZeroDucatOffers`, each defaulting to false. The title-owned normalizer accepts the historical `publicMoney`, `playerPlacedCastles`, and `minimumOneDucat` keys and inverts their values. Missing legacy values retain the ordinary game. Legacy keys take precedence when both formats are present because older site forms can insert current defaults without translating saved choices. Normalized configurations omit legacy aliases, so subsequent edits use only current keys. Stored state still uses `publicMoney` and `minimumOneDucat`, so existing state and action history need no migration.

Forms normalize before showing values or merging defaults, and tournament validation normalizes before adding defaults. Cards, tournament summaries, and public invitation previews use the same conversion. Löwenherz registers its configurator in Info and constructs its definition with `defineGame`; shared runtime and client-context entry points normalize configuration for initialization, Action availability, Actions and State Handlers, replay, Exploration, and visibility. Its rule and session code read the current keys directly. An unchanged save of a legacy private-money setup must remain private.

Deploy the Site Frontend and backend support for optional `GameConfigurator.normalizeConfig` before publishing the matching Löwenherz Logic and UI Artifacts. Older title artifacts omit the hook and retain their existing behavior. The new GameEditForm and GameContext are bundled into UI Artifacts. Publish the matching Löwenherz Logic and UI Artifacts to adopt this migration. Other titles retain existing behavior without a normalization hook; each title that later adopts configuration migration must publish its own matching Logic and UI Artifacts. A Site Frontend deployment alone does not update bundled Game Client code. Refresh older site tabs before editing legacy setups with the new title definitions. The release workflow owns version selection and bumps. Release must establish a logic major-version boundary so already-loaded clients that cannot interpret the new configuration must reload. Earlier logic cannot interpret newly saved configuration, so rollback requires reverse config compatibility or a forward fix.

...
A standing declaration a Player may make outside their turn is an Out-of-Turn Action: its schema requires the literal `outOfTurn: true`, the engine skips the active-player and index checks for it, and its handler alone validates it. See [ADR 0007](adr/0007-out-of-turn-actions.md). Automatic rule consequences are first-class System Actions created or scheduled through `MachineContext`. A Svelte effect or other reactive UI loop must never commit gameplay. Gameplay-relevant mutation belongs inside runtime processing so history, undo, replay, and remote clients observe the same sequence.

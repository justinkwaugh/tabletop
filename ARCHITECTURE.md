...
**Specific Games**
Game implementations are intended be very modular. They are split into two parts for dependency management and deployment reasons.

- The first part is a module that provides the model and logic of the game. This is comprised of the game state, state handlers, actions, and configuration / setup data and so forth. The module has to implement and export a `GameDefinition` object named `Definition` for the backend to be able to support it. It is comprised of a `GameInfo` object and a `GameRuntime` object. It is bundled and served as a single file for the backend to dynamically load.
- The second part is a module that provides the game UI. There is very little constraint on the UI itself, though over time I'm sure certain patterns will be factored out into components that may be shared between games, but in general a game gets the entire page space below the toolbar to express the game in any manner it desires. The UI module has to implement and export a `GameUiDefinition` object named `UiDefinition` for the frontend to be able to support it. The definition provides two pieces of information to the frontend, namely a `GameUIInfo` object which extends `GameInfo` and a promise that returns a `GameUIRuntime` which extends `GameRuntime` adding a `GameSession` subclass and a Svelte based `DynamicComponent` which implements the UI for the game. The frontend will render that component and provide an instance of the session class to it. Beyond that component there is no specific requirement to use Svelte. This is bundled with the game logic included (the frontend also runs the game logic) and served statically by the backend.

Because the games are modular and dynamically loaded from static bundles, they could very easily be implemented in separate packages/repositories independent of this project.

...
for (const engine of ['chromium', 'webkit']) {
    test(`${engine} centers fixed and staged revenue labels`, async ({ page, baseURL }) => {
        ...
        try {
            ...
            const centers = await labels.evaluateAll(elements => elements.map(element => {
                if (!(element instanceof SVGGraphicsElement)) throw new Error('Expected SVG revenue text')
                const box = element.getBBox()
                return { text: element.textContent, x: box.x + box.width / 2, y: box.y + box.height / 2 }
            }))

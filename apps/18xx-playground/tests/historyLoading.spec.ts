...
for (const newestFirst of [false, true]) {
    test(`async history opens at the newest end with newest ${newestFirst ? 'first' : 'last'}`, async ({
        page
    }) => {
        await page.goto('/')
        await page.evaluate(async (first) => {
            const fixture = await import(
                new URL('/src/demo/historyLoading.fixture.ts', location.href).href
            )
        ...
        await page
            .getByRole('button', {
                name: newestFirst ? 'Newest last' : 'Newest first',
                exact: true
            })
            .click()
        await expect

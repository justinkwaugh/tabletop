...
test('player order moves when navigating between recorded turn orders', async ({page}) => {
    ...
    const before = await page.locator('.players article[data-player-id]').evaluateAll(nodes => nodes.map(n => n.getAttribute('data-player-id')))
    await page.evaluate(() => {
        const animate = Element.prototype.animate
        Element.prototype.animate = function (...args) {
            if (this.matches('.players article')) this.setAttribute('data-observed-animation', JSON.stringify(args[1]))
    ...
    for (let step = 0; step < 180 && !changed; step++) {
        ...
        const order = await page.locator('.players article[data-player-id]').evaluateAll(nodes => nodes.map(n => n.getAttribute('data-player-id')))

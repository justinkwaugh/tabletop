...
async function withholdThroughOperatingTurn(page: Page) {
    ...
    const step = action
        .getByRole('button', {
            name: /^(skip|Run trains|finish|Continue operating round)$|^Withhold/
        })
        .first()
    for (;;) {
...
for (const title of ['TOP', '1889']) {
    test(`${title} spreadsheet records OR income and keeps completed valuations through reload and Undo`, async ({
        page
    }) => {
        const errors: string[] = []
        ...
        await expect(
            page.getByText('No operating-round history recorded yet.', { exact: true })
        ).toBeVisible()
        ...
        await expect(
            history.getByRole('columnheader', { name: 'Income', exact: true })
        ).toHaveCount(3)
        await expect(
            history.getByRole('columnheader', { name: 'Net worth', exact: true })
        ).toHaveCount(3)
...
for (const title of ['TOP', '1889']) {
    test(`${title} history separates gross train revenue from player dividends`, async ({
        page
    }) => {
        ...
        await expect(steps.getByRole('button', { name: /^Run/ })).toContainText(
            `Ran for ${revenue}`
        )
        ...
        await page.getByRole('tab', { name: 'Spreadsheet', exact: true }).click()
        await page
            .getByRole('group', { name: 'Spreadsheet period' })
            .getByRole('button', { name: 'Income', exact: true })
            .click()
        const history = page.getByRole('table', { name: 'Operating round history' })
        const round = history
            .getByRole('row')
            .filter({ has: page.getByRole('rowheader', { name: 'OR 1.1 *', exact: true }) })

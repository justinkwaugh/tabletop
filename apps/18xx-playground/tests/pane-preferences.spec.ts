...
test('layout edits debounce, recover on immediate reload, and restore split percentages', async ({ page }) => {
    ...
    await expect.poll(() => storedFamilyPreference(page, 'paneLayout'), { timeout: 10000 }).toMatchObject({ v: 1, main: ['cols', 20, ['rows', 25, ['Game info'], ['Players', 'History', 'Chat']], ['rows', 46, ['Actions'], ['Map', 'Market', 'Spreadsheet', 'Companies', 'Tiles', 'Player Aid']]] })
...
test('restores tab transfers and sanitizes unknown tabs without overwriting future layouts', async ({ page }) => {
    ...
    await expect.poll(() => storedFamilyPreference(page, 'paneLayout')).toMatchObject({ sidebar: [], main: ['cols', 20, ['rows', 25, ['Game info'], ['Players', 'Chat']], ['rows', 50, ['Actions'], ['History', 'Map', 'Market', 'Spreadsheet', 'Companies', 'Tiles', 'Player Aid']]] })
    await page.reload()
...
test('swap exchanges whole panes, preserves mounted content and saves the result', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Layout unsaved', exact: true }).click()
    await expect.poll(() => storedFamilyPreference(page, 'paneLayout')).toMatchObject({ main: ['cols', 20, ['rows', 25, ['Game info'], ['Players', 'History', 'Chat']], ['rows', 80, ['Map', 'Market', 'Spreadsheet', 'Companies', 'Tiles', 'Player Aid'], ['Actions']]] })
    await page.reload()

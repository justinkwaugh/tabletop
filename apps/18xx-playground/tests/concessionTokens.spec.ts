...
test('TOP share-granting private cards show the company token and bold certificate introduction', async ({ page }) => {
    await page.goto('/table')
    await page.getByLabel('Position', { exact: true }).selectOption('opening')
    const offers = page.getByRole('region', { name: 'Auction offers' })
    const names = ['Mainline Concession', 'Shortline Concession', 'Royal Agricultural Society', 'Railcar Ferry']

...
for (const browserName of ['chromium', 'firefox'] as const) {
    test.describe(browserName, () => {
        test('outlines only the operating company and crosses dividers continuously', async ({
            baseURL
        }) => {
            ...
            try {
                ...
                if (!(await table.locator('.operating-column').count()))
                    await page.getByRole('button', { name: 'Swap rows and columns' }).click()
                await table.evaluate((element) =>
                    element.parentElement?.style.setProperty('--rail-focus', '#ff00ff')
                )
                    ...
                    await expect(async () => {
                        const points = await table.evaluate((element) => {
                            ...
                            const visibleRight = Math.min(
                                bounds.right,
                                element.closest('.table-scroll')?.getBoundingClientRect().right ??
                                    bounds.right
                            )
                            if (
                                element.querySelectorAll(
                                    'thead .current-player-column, tr.current-player'
                                ).length
                            )
                                throw new Error(
                                    'Current player must not be outlined while a company operates'
                                )
                            const column = element
                                .querySelector('thead .operating-column')
                                ?.getBoundingClientRect()
                            const rows = [...element.querySelectorAll('tr.operating-company')].map(
                                (row) => row.getBoundingClientRect()
                            )
                            if ((column ? 1 : 0) + rows.length !== 1)
                                throw new Error('Exactly one operating company outline expected')
                            const points: {
                                x: number
                                y: number
                                vertical: boolean
                                ...
                                return points
                                    .filter((point) => {
                                        ...
                                        for (let offset = -1; offset <= 2; offset++) {
                                            const x =
                                                Math.floor(point.x) + (point.vertical ? offset : 0)
                                            const y =
                                                Math.floor(point.y) + (point.vertical ? 0 : offset)

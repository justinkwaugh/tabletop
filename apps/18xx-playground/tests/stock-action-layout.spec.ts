...
for (const title of ['TOP', '1889']) {
    test(`${title} stock actions center without overlapping Pass`, async ({ page }) => {
        ...
        for (const width of [700, 360, 320, 240, 200, 180, 700]) {
            await strip.evaluate((element, width) => {
                ...
                .poll(async () =>
                    strip.evaluate((element) => {
                        ...
                        const reserved =
                            ...
                            pass.left +
                            parseFloat(
                                getComputedStyle(element.querySelector('.commit')!).marginLeft
                            )
                        const fits = pills.width + 2 * reserved <= bounds.width
                        const center = bounds.left + (bounds.width - (fits ? 0 : reserved)) / 2
                        return (
                            Math.abs(pills.left + pills.width / 2 - center) < 1 &&
                            pills.left >= bounds.left &&
                            pills.right <= pass.left - 7 &&
                            [...element.querySelectorAll('.selections button')].every(

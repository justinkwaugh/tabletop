...
import { TrainPurchase, type BuyTrain } from '@tabletop/18xx'
import { example } from './stockTestUtils.js'

it.each([
    { definition: Top, rules: TheOldPrinceTrainRules },
    { definition: Shikoku, rules: Shikoku1889TrainRules }
])(
    'ends train buying when cash exhaustion leaves no further choices: $definition.info.id',
    ({ definition, rules }) => {
        const { game, engine, state } = example(definition, 'trains')
        const details = new TrainPurchase(state, rules)
            .offers()
        ...
        const result = engine.executeCanonicalAction({ game, state, action: buy })
        expect(
            result.processedActions.some(
                (action) =>
                    action.type === 'FinishOperatingTurn' && action.source === ActionSource.System
            )
        ).toBe(true)
        let reversed = result.updatedState
        for (const action of result.processedActions.toReversed())
            reversed = engine.undoProcessedAction({ state: reversed, action })

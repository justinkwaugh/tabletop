...
`TABLETOP_BACKEND_REGION`, `TABLETOP_INVESTIGATE_CREDENTIAL_FILE`.
...
| `tasks [--queue name] [--limit 50]`                                                                                                                              | Queues, or the still-pending tasks in one queue with decoded bodies; `stuck` marks a task more than five minutes past its schedule                                   |
...
`token`, and converts Firestore timestamps to ISO strings.
